...
public class MssageController  {
    ...
    Dto doRegister(
        ...
        if(!validPhone(userVO.getUserCode()))
            return  DtoUtil.returnFail("请使用正确的手机地址注册",ErrorCode.AUTH_ILLEGAL_USERCODE);

        try {
            System.out.print("======================短信注册 进来了");
    ...
    public @ResponseBody
    Dto validatephone(
            @ApiParam(name="user",value="注册手机地址",defaultValue="test@bdqn.cn")
